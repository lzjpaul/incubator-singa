...
void RBMVisLayer::ComputeFeature(Phase phase, Metric* perf) {
  ...
  if (phase == kPositive) { /*positive phase*/
        ...
        Tensor<cpu, 2> src(srclayers_[data_idx_]->mutable_data(this)->mutable_cpu_data(),
         Shape2(batchsize_, vdim_));
        Copy(data, src);
  }
        ...
        if (is_first_iteration_vis_) {
          ...
          is_first_iteration_vis_ = false;
            ...
            Tensor<cpu, 2> hid_sample(srclayers_[hid_idx_]->mutable_data(this, kNegative)->mutable_cpu_data(),
...
void RBMVisLayer::ComputeLoss(Metric* perf) {
  ...
  CHECK_EQ(srclayers_[data_idx_]->data(this).count(), batchsize_*vdim_);
          Tensor<cpu, 2> src(srclayers_[data_idx_]->mutable_data(this)->mutable_cpu_data(),
          Shape2(batchsize_, vdim_));
  Tensor<cpu, 2> hid_data(srclayers_[hid_idx_]->mutable_data(this, kPositive)->mutable_cpu_data(), //gibbs using u
              Shape2(neg_batchsize_, hdim_));
...
void RBMHidLayer::ComputeFeature(Phase phase, Metric* perf) {
  ...
  if (phase == kPositive) {  /*postive phase*/
    ...
    Tensor<cpu, 2> src_sample(srclayers_[0]->mutable_data(this, kNegative)->mutable_cpu_data(),
         Shape2(neg_batchsize_, vdim_));
