            ...
            for j in range(len(self.a_idx_list)):
                for k in range(len(self.alpha_idx_list)):
                    for l in range(len(self.b_idx_list)):
                        ...
                        hyper_param_path = "8-22/hyperparams" # modify here
                        hyper_param_prefix = 'lambda_' + str(i) + '_a_' + str(j) + '_alpha_' + str(k) + '_b_' + str(l)
                        data_path = "/data/zhaojing/regularization/log0822/GMM-DL-four" # modify here
                        script = ("python " + python_programme + "-maxepoch 10 -gmnum 4 -gmuptfreq 100 -paramuptfreq 50 -gpuid 0 -hyperparampath " + str(os.path.join(hyper_param_path, (hyper_param_prefix + ".csv"))) + " -resultpath " + str(os.path.join(data_path, (hyper_param_prefix + "_result.csv "))) + "resnet cifar-10-batches-py/ | tee -a " + data_path + "/GMM-DL-four-" + str(index) + ".log\n") # modify here
...
if __name__ == '__main__':
   cs = Config_Script()
   cs.gen_hyperparam_config('8-22/hyperparams')
   cs.gen_sh_script('8-22/scripts/GMM-DL-four.sh') # need to go into to modify
   cs.gen_multiple_sh_scripts('8-22/scripts/GMM-DL-four.sh', '8-22/scripts/GMM-DL-four-')
