    ...
    def extract_layer_name_dim_gmregularizer(self, net, hyperpara, gm_num, pi, reg_lambda, uptfreq):
        for (s, p) in zip(net.param_names(), net.param_values()):
            ...
            if np.ndim(tensor.to_numpy(p)) == 2:
                ...
                hyperpara = self.layer_wise_hyperpara(dims) # layerwise initialization of hyper-params
                self.gmregularizers[s] = GMRegularizer(hyperpara=hyperpara, gm_num=gm_num, pi=pi, reg_lambda=reg_lambda, uptfreq=uptfreq)
...
class GMRegularizer(Regularizer):
    ...
    def apply(self, dev, trainnum, net, epoch, value, grad, name, step):
        ...
        grad = tensor.from_numpy((self.reg_grad_w.reshape(tensor.to_numpy(value).shape[0], -1))/float(trainnum))
        grad.to_device(dev)
        print "epoch < 2 or step % self.paramuptfreq == 0: ", epoch < 2 or step % self.paramuptfreq == 0
