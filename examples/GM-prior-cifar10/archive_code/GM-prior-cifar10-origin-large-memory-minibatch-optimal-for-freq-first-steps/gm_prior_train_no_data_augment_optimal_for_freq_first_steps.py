...
def train(resultpath, data, model_name, hyperpara_list, hyperpara_idx, gm_num, gm_lambda_ratio, firstepochs, uptfreq, net, max_epoch, get_lr, weight_decay, gpuid, batch_size=100,
          use_cpu=False):
    ...
    est = datetime.datetime.fromtimestamp(epoch_start).strftime('%Y-%m-%d %H:%M:%S')
    print "epoch_start time: ", est
    for epoch in range(max_epoch):
        ...
        for b in range(num_train_batch):
            ...
            for (s, p, g) in zip(net.param_names(), net.param_values(), grads):
                opt.apply_with_lr(dev=dev, trainnum=train_x.shape[0], net=net, epoch=epoch, firstepochs=firstepochs, lr=get_lr(epoch), grad=g, value=p, name=str(s), step=b)
            # update progress bar
        ...
        edo = datetime.datetime.fromtimestamp(epoch_done).strftime('%Y-%m-%d %H:%M:%S')
        print "epoch_done time: ", edo
        epoch_elapsed = epoch_done - epoch_start
        print "epoch_elapsed time: ", epoch_elapsed 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-firstepochs', type=int, help='first epochs for not lazy update')
        ...
        for alpha_idx in alpha_idx_arr:
            for b_idx in b_idx_arr:
                for a_idx in a_idx_arr:
                    ...
                    train(args.resultpath, (train_x, train_y, test_x, test_y), args.model, [a_list, b_list, alpha_list], [a_idx, b_idx, alpha_idx], gm_num, gm_lambda_ratio, args.firstepochs, [args.gmuptfreq, args.paramuptfreq], net, 160, alexnet_lr, 0.004, args.gpuid, use_cpu=args.use_cpu)
                    done = time.time()
                    do = datetime.datetime.fromtimestamp(done).strftime('%Y-%m-%d %H:%M:%S')
                    print do
                    elapsed = done - start
