...
class GMOptimizer(Optimizer):
    ...
    def apply_GM_regularizer_constraint(self, dev, trainnum, net, epoch, firstepochs, value, grad, name, step):
        # if np.ndim(tensor.to_numpy(value)) <= 2:
        if np.ndim(tensor.to_numpy(value)) != 2:
            return self.apply_regularizer_constraint(epoch, value, grad, name, step)
        else: # weight parameter
            grad = self.gmregularizers[name].apply(dev, trainnum, net, epoch, firstepochs, value, grad, name, step)
            return grad
...
class GMRegularizer(Regularizer):
    ...
    def apply(self, dev, trainnum, net, epoch, firstepochs, value, grad, name, step):
        self.w_array = tensor.to_numpy(value).reshape((-1, 1)) # used for EM update also
        if epoch < firstepochs or step % self.paramuptfreq == 0:
        ...
        if (epoch == 0 and step < 50) or step % self.gmuptfreq == 0:
            print "first epochs: ", firstepochs
            print "step: ", step
            print "name: ", name
        ...
        if epoch < firstepochs or step % self.gmuptfreq == 0:
            if epoch >=firstepochs and step % self.paramuptfreq != 0:
                self.calcResponsibility()
...
class GMSGD(GMOptimizer, SGD):
    ...
    def apply_with_lr(self, dev, trainnum, net, epoch, firstepochs, lr, grad, value, name, step=-1):
        if grad.is_empty():
            return value
        ##### GM-prior: using gm_regularizer ##############
        grad = self.apply_GM_regularizer_constraint(dev=dev, trainnum=trainnum, net=net, epoch=epoch, firstepochs=firstepochs, value=value, grad=grad, name=name, step=step)
        ##### GM-prior: using gm_regularizer ##############
