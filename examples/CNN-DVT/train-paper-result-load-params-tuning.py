...
def train(inputfolder, outputfolder, visfolder, sampleid, dev, agent, max_epoch, use_cpu, batch_size=100):
    ...
    # stride_y = 1
    # stride_x = 20
    hyperpara = np.array([12, 375, 2, 15, 2, 5])
    ...
    net.load(os.path.join(outputfolder, 'model-checkpoints/model-time-2017-10-30-17-04-22'), 20)
    net.to_device(dev)
    ...
    explain_occlude_area_format_out(sampleid, visfolder, np.copy(test_feature), np.copy(test_label), os.path.join(outputfolder,'readmitted_prob.csv'), os.path.join(outputfolder,'true_label_prob_matrix.csv'), os.path.join(outputfolder,'meta_data.csv'), top_n = 20)
