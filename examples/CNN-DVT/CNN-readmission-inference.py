...
def train(inputfolder, outputfolder, visfolder, sampleid, dev, agent, max_epoch, use_cpu, batch_size=100):
    ...
    net.load(os.path.join(outputfolder, 'parameter_last240'), 20)
    net.to_device(dev)
        ...
        if epoch % test_epoch == 0 or epoch == (max_epoch-1):
            ...
            try:
                with open(os.path.join(visfolder, 'cnn_metric_info.json'), 'w') as cnn_metric_info_writer:
                    ...
                    cnn_metric_info_writer.write('\"%s\": %d, '%("number of samples: ", y.shape[0]))
                    cnn_metric_info_writer.write('\"%s\": %04.2f, '%("AUC: ", auroc(softmax(tensor.to_numpy(probs))[:,1].reshape(-1, 1), y.reshape(-1, 1))))
                    cnn_metric_info_writer.write('\"%s\": %04.2f, '%("sensitivity: ", cnn_sensitivity))
                    cnn_metric_info_writer.write('\"%s\": %04.2f'%("specificity: ", cnn_specificity))
                    cnn_metric_info_writer.write('}')
