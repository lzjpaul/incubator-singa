...
# to lasso (done) && grad is not yet (done) done && huber has (w + v) (done)
...
def huber_grad_descent_avg(batch_X, batch_y, w, v, param, C, is_l1):
    ...
    tgrad = param * ((tv > 0.) - (tv < 0.)) if is_l1 else param * 2.0 * tw
    ...
    tf1 = tensor.exp( ((-1) * tbatch_y) * tensor.mult((tw + tv), tbatch_X.T()) )
    tres = tbatch_X
...
def elasticnet_grad_descent_avg(batch_X, batch_y, w, param, l1_ratio_or_mu, C, dev):
    ...
    tgrad = param * l1_ratio_or_mu * ((tw > 0.) - (tw < 0.)) + param * (1 - l1_ratio_or_mu) * tw
...
def huber_optimizator_avg(X, y, lambd, l1_ratio_or_mu, C, max_iter, eps, alpha, decay, batch_size, clf_name):
    ...
    while True:
        ...
        # v -= alpha * grad_descent_avg(batch_X, batch_y, w, v, l1_ratio_or_mu, C, True, dev)
        tvgd_avg = grad_descent_avg(batch_X, batch_y, w, v, l1_ratio_or_mu, C, True, dev)
        tvgd_avg.to_device(cpudev)
        vgd_avg = tensor.to_numpy(tvgd_avg)
        ...
        wgd_avg = tensor.to_numpy(twgd_avg)
