...
class Elasticnet_Classifier(BaseEstimator, LogisticLinearClassifierMixin):
    ...
    def fit(self, X_train, y_train, X_test, y_test):
        ...
        #                                                self.max_iter, self.eps, self.alpha, self.decay, self.batch_size)
            ...
            # print "self.batch_size: ", self.batch_size
        self.n_iter_, self.w_ = optimizator_gd.non_huber_optimizator_avg(X_train, y_train, X_test, y_test, self.lambd, self.l1_ratio, self.C,
                                                    (y_train.shape[0] * 10 / self.batch_size), self.eps, self.alpha, self.decay, self.batch_size, 'elasticnet')
