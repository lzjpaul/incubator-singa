...
class Gaussian_Mixture_Regularization(BaseEstimator, LogisticLinearClassifierMixin):
    ...
    def fit(self, X_train, y_train, X_test, y_test, batchgibbs):
        ...
        print "in Gaussian_Mixture_Regularization fit"
        ...
        print "self.batch_size: ", self.batch_size
        self.n_iter_, self.w_, self.best_accuracy_, self.best_accuracy_step_ = optimizator_gd.gaussian_mixture_optimizator_avg(X_train, y_train, X_test, y_test, self.C,
                                                     (y_train.shape[0] * 10 / self.batch_size), self.eps, self.alpha, self.n_gaussian, self.theta_alpha, self.a, self.b, self.decay, self.batch_size, 'gaussianmixture', batchgibbs, initial_L2_lambd=0.1, initial_L2_alpha=0.1, initial_L2_step=120)
        self.coef_ = self.w_.reshape((1, X_train.shape[1]))
        self.intercept_ = 0.0
        if self.fit_intercept:
            self.intercept_ = self.coef_[:,-1]
            self.coef_ = self.coef_[:,:-1]
        print "in Gaussian_Mixture fit"
    ...
    def predict_proba(self, X_train):
        return super(Gaussian_Mixture_Regularization, self).decision_function(X_train)
