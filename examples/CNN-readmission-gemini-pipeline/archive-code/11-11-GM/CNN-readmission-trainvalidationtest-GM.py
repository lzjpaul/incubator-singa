...
def train(inputfolder, outputfolder, visfolder, trainratio, validationratio, testratio, dev, agent, max_epoch, use_cpu, batch_size=100):
    ...
    net.to_device(dev)
    opt = gm_prior_optimizer.GMSGD(net=net, momentum=0.9, weight_decay=0.01)
    gm_lambda_ratio = 0.0
    a_list = [10.] # a = 10 * b, so lambda = 10; also, need to lambda / num_train_samples, so final is 0.01
    b_list, alpha_list = [10000.], [0.9]
    a_idx = 0
    b_idx = 0
    alpha_idx = 0
    hyperpara_list, hyperpara_idx = [a_list, b_list, alpha_list], [a_idx, b_idx, alpha_idx]
    gm_num = 4
    uptfreq = [100, 50]
    for (p, specs) in zip(net.param_names(), net.param_specs()):
        opt.register(p, specs)
    for (s, p) in zip(net.param_names(), net.param_values()):
        opt.gm_register(s, p, 'alexnet', hyperpara_list, hyperpara_idx, gm_num, gm_lambda_ratio, uptfreq)
    ...
    for epoch in range(max_epoch):
        ...
        for b in range(num_train_batch):
            ...
            for (s, p, g) in zip(net.param_specs(),
                ...
                opt.apply_with_lr(dev=dev, trainnum=trainx.shape[0], net=net, epoch=epoch, lr=0.005, grad=g, value=p, name=str(s.name), step=b)
