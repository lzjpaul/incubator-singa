...
def train(data, net, max_epoch, get_lr, weight_decay, n_gaussian=3, theta_alpha=1, a=1, b=1, C=1.0, batch_size=100, 
          use_cpu=False):
    ...
    for epoch in range(max_epoch):
        ...
        for b in range(num_train_batch):
            ...
            for (s, p, g) in zip(net.param_names(), net.param_values(), grads):
                ...
                if str(s).contains('weight'):
                    if len(weight_dim_list) == 1:
                        weight_array = tensor.to_numpy(p).reshape((1, -1))
                    else:
                        weight_array = np.concatenate((weight_array, tensor.to_numpy(p).reshape((1, -1))), axis=1)
                p.to_device(dev)
            ...
            for (s, p, g) in zip(net.param_names(), net.param_values(), grads):
                ...
                param_regularization_grad = tensor.from_numnpy(w_array[weight_index:(weight_index+param_dim_0 * param_dim_1)].reshape(weight_dim_list[weight_dim_list_index]))
            ...
            opt.apply_with_lr(epoch, get_lr(epoch), t_theta_r_vec_update, t_theta_r_vec, 't_theta_r_vec', b)
            opt.apply_with_lr(epoch, get_lr(epoch), t_lambda_t_vec_update, t_lambda_t_vec, 't_lambda_t_vec', b)
