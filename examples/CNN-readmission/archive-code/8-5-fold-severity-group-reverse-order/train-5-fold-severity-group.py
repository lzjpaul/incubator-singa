...
# just use the best parameters to train
# just need to change the path of input data
...
def train(dev, agent, max_epoch, use_cpu, batch_size=100):
    ...
    kernel_x_param_array = np.array([10])
    stride_y_param_array = 2
    stride_x_param_array = np.array([3, 3])
    all_feature, all_label = get_data\
    ('/data/zhaojing/regularization/LACE-CNN-1500/severity-group/nuh_fa_readmission_case_demor_inpa_kb_ordered_output_severity_onehot_12slots.csv', \
