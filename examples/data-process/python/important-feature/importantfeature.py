...
presigmoid_dup = np.genfromtxt(file, delimiter=",")
file.close()
...
for k in range(len(presigmoid)):
    ...
    for i in range(len(presigmoid_dup)):
        if presigmoid_dup[i] == sigmoid_value and j < selected_sample_num:
...
file.close()
#python importantfeature.py tryimportantfeaturepresigmoid.csv tryimportantfeatureinputsample.csv 3 3 tryimportantfeature_selected_feature_index.csv
