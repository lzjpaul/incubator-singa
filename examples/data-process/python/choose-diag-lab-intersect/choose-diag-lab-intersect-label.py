...
# train_data_matrix only has one column!! line 35
# data type is int
import numpy as np
import numpy
import sys
...
train_data_matrix = train_data_matrix.astype(np.int)
file.close()
print ("train_data_matrix = ", train_data_matrix.shape)

if len(train_index) != len(train_data_matrix):
...
intersect_train_data_matrix = np.zeros(len(intersect_index))
...
#output
a = numpy.asarray(intersect_train_data_matrix, dtype = np.int)
numpy.savetxt(sys.argv[4], a, fmt = '%d', delimiter=",") #modify here
# python choose-diag-lab-intersect-label.py NUHDSSOCREADMI_DIAGLAB_ENGI_SUB_IDX_LAB_SID.csv NUHDSSOCREADMI_DIAGLAB_ENGI_SUB_IDX_DIAG_LAB_INTERSECT_SID.csv /ssd/zhaojing/lab-laplacian/idx-case/all-case-idx-case/NUH_DS_SOC_READMISSION_CASE_LAB_idxcase_label.csv /ssd/zhaojing/lab-laplacian/idx-case/all-case-idx-case/intersect/NUH_DS_SOC_READMISSION_CASE_LAB_idxcase_label_INTERSECT.txt
