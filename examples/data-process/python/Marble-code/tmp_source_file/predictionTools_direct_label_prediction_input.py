...
modelPred = np.genfromtxt(file, delimiter=",")
modelPred = modelPred.astype(np.float)
fpr, tpr, thresholds = metrics.roc_curve(testY.T, modelPred, pos_label=1)
auc = metrics.auc(fpr, tpr)
print "auc = \n", auc


y_true, y_scores = testY.T.astype(np.int), modelPred.astype(np.float) #modify here0:1018!!!!!!! less than 1018
# print "y_true shape = \n", y_true.shape
