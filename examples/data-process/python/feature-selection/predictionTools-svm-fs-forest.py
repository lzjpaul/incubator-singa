...
selected_fea_num = int(sys.argv[5])
...
print "importances = \n", importances
indices = np.argsort(importances)[::-1][:selected_fea_num]
print "indices = \n", indices

trainX_new = np.zeros([len(trainX[:,0]),selected_fea_num])
testX_new = np.zeros([len(testX[:,0]),selected_fea_num])

new_train_fea_index = 0
for i in range(len(trainX[0,:])):
    if i in indices:
        trainX_new[:,new_train_fea_index] = trainX[:,i]
        new_train_fea_index = new_train_fea_index + 1
print "final new_train_fea_index = \n", new_train_fea_index

new_test_fea_index = 0
for i in range(len(testX[0,:])):
    if i in indices:
        testX_new[:,new_test_fea_index] = testX[:,i]
        new_test_fea_index = new_test_fea_index + 1
print "final new_test_fea_index = \n", new_test_fea_index
        
trainX = trainX_new
testX = testX_new 
print "transformed trainX shape = \n", trainX.shape
print "transformed testX shape = \n", testX.shape
