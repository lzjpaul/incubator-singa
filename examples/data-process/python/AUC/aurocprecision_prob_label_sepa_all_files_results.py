...
def calAUC(valid_num, test_num, all_label_file, all_prob_file, valid_output, test_output):
    ...
    return pre_AUC_matrix_test[best_validataion_place,1]
...
f = open('/data/zhaojing/result/infoversion' + str(random.randint(0,20000)), 'w+')
for i in range(len(arr_label)):
    print "i is = ", i
    f.write("i is = " + str(i) + "\n")
    ...
    best_AUC.append(calAUC(2000, 3000, arr_label[i], arr_prob[i], "/data/zhaojing/result/valid_" + str(i) + ".csv", "/data/zhaojing/result/test_" + str(i) + ".csv"))
