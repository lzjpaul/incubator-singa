...
list_dirs = os.walk(sys.argv[2]) #prob
for root, dirs, files in list_dirs:
    for f in files:
        # print os.path.join(root, f)
...
for i in range(len(arr_label)):
    ...
    print "i is = ", i
    calAUC(2000, 3000, arr_label[i], arr_prob[i], "/data/zhaojing/AUC/valid_" + str(i) + ".csv", "/data/zhaojing/AUC/test_" + str(i) + ".csv")
# python aurocprecision_prob_label_sepa_all_files.py /data/zhaojing/AUC/label/ /data/zhaojing/AUC/prob/
