...
data_dir_prefix = '/data/zhaojing/result/1-19-CNN-2layer-2src-PEARSON-3'
...
pad_x_param = 0
pad_y_param = 0
filter_num_param_array = np.array([150, 200, 250, 300])
filter_num_param = filter_num_param_array[random.randint(0,len(filter_num_param_array)-1)]

#second conv layer
kernel_x_param_array_2 = np.array([3, 5, 10, 15, 20])
kernel_y_param_array_2 = np.array([3])
stride_x_param_array_2 = np.array([1, 2, 3])
stride_y_param_array_2 = np.array([2, 3])
kernel_x_param_2 = kernel_x_param_array_2[random.randint(0,len(kernel_x_param_array_2)-1)]
kernel_y_param_2 = kernel_y_param_array_2[random.randint(0,len(kernel_y_param_array_2)-1)]
if kernel_x_param_2 < 5:
    stride_x_param_2 = stride_x_param_array_2[random.randint(0,0)]
else:
    stride_x_param_2 = stride_x_param_array_2[random.randint(0,len(stride_x_param_array_2)-1)]
stride_y_param_2 = stride_y_param_array_2[random.randint(0,len(stride_y_param_array_2)-1)]
pad_x_param_2 = 0
pad_y_param_2 = 0
filter_num_param_array_2 = np.array([300, 350, 400, 450, 500])
filter_num_param_2 = filter_num_param_array_2[random.randint(0,len(filter_num_param_array_2)-1)]
...
#calculate 1st layer fan_in and fan_out
conv_fan_in = input_channel * kernel_y_param * kernel_x_param
conv_fan_out = filter_num_param * kernel_y_param * kernel_x_param 
conv_fan_out_1 = (filter_num_param * kernel_y_param * kernel_x_param) / (kernel_x_param_2 * kernel_y_param_2)
...
#calculate 2nd layer fan_in and fan_out
conv_fan_in_2 = filter_num_param * kernel_y_param_2 * kernel_x_param_2
conv_fan_out_2 = filter_num_param_2 * kernel_y_param_2 * kernel_x_param_2 
conv_fan_out_2_1 = (filter_num_param_2 * kernel_y_param_2 * kernel_x_param_2) / (pool_x_param * pool_y_param)

conv_n_array_2 = np.array([conv_fan_in_2, (conv_fan_in_2 + conv_fan_out_2)/2, conv_fan_out_2, (conv_fan_in_2 + conv_fan_out_2_1)/2, conv_fan_out_2_1])
...
#gaussian
conv_std_w_param_2 = numpy.sqrt(2. / (conv_n_array_2[random.randint(0,len(conv_n_array_2)-1)]))
conv_constant_b_param_2 = 0
#uniform, b can use 0 also
conv_uniform_w_param_2 = numpy.sqrt(3. / (conv_n_array_2[random.randint(0,len(conv_n_array_2)-1)]))
conv_uniform_b_param_2 = conv_uniform_w_param_2
...
feamap_y_2 = (feamap_y - kernel_y_param_2 + 2*pad_y_param_2) / stride_y_param_2 + 1
feamap_x_2 = (feamap_x - kernel_x_param_2 + 2*pad_x_param_2) / stride_x_param_2 + 1
pool_map_y = (feamap_y_2 - pool_y_param + 2*pool_pad_y_param) / pool_stride_y_param + 1
pool_map_x = (feamap_x_2 - pool_x_param + 2*pool_pad_x_param) / pool_stride_x_param + 1
...
# second layer param
#gaussian
conv_parw_gaussian_2 = Parameter(init='gaussian', std=conv_std_w_param_2)
conv_parb_gaussian_2 = Parameter(init='constant', value=conv_constant_b_param_2)
#uniform
conv_parw_uniform_2 = Parameter(init='uniform', scale=conv_uniform_w_param_2)
conv_parb_uniform_2 = Parameter(init='uniform', scale=conv_uniform_b_param_2)
conv_parb_uniform_2_1 = Parameter(init='constant', value=conv_constant_b_param_2)
...
if Uniform_or_Gaussian == 1:
    m.add(Convolution2D(filter_num_param_2, pad_x=pad_x_param_2, pad_y=pad_y_param_2, stride_x=stride_x_param_2, stride_y=stride_y_param_2, kernel_x=kernel_x_param_2, kernel_y=kernel_y_param_2, w_param=conv_parw_gaussian_2, b_param=conv_parb_gaussian_2))
elif Uniform_or_Gaussian == 0 and b_Uniform_or_Constant == 0:
    m.add(Convolution2D(filter_num_param_2, pad_x=pad_x_param_2, pad_y=pad_y_param_2, stride_x=stride_x_param_2, stride_y=stride_y_param_2, kernel_x=kernel_x_param_2, kernel_y=kernel_y_param_2, w_param=conv_parw_uniform_2, b_param=conv_parb_uniform_2))
elif Uniform_or_Gaussian == 0 and b_Uniform_or_Constant == 1:
    m.add(Convolution2D(filter_num_param_2, pad_x=pad_x_param_2, pad_y=pad_y_param_2, stride_x=stride_x_param_2, stride_y=stride_y_param_2, kernel_x=kernel_x_param_2, kernel_y=kernel_y_param_2, w_param=conv_parw_uniform_2, b_param=conv_parb_uniform_2_1))
m.add(Activation('relu'))

m.add(MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param))

if Add_MLP_or_not == 1:
    layer_neuron_size_array = np.array([100, 500, 1000])
    mlp_fan_in = filter_num_param_2 * pool_map_y * pool_map_x
    mlp_fan_out = layer_neuron_size = layer_neuron_size_array[random.randint(0,len(layer_neuron_size_array)-1)]
    m.add(Dense(layer_neuron_size, init='uniform', scale=numpy.sqrt(6. / (mlp_fan_in + mlp_fan_out)), activation='relu'))
    ...
    softmax_fan_in = filter_num_param_2 * pool_map_y * pool_map_x
    softmax_fan_out = 2
    softmax_n_array = np.array([softmax_fan_in,(softmax_fan_in + softmax_fan_out)/2]) # do not use fan_out
