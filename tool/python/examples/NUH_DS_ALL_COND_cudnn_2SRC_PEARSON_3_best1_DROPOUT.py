...
pad_x_param = 2
pad_y_param = 0
filter_num_param_array = np.array([500, 700])
...
softmax_fan_out = 2
softmax_n_array = np.array([(softmax_fan_in + softmax_fan_out)/2]) # do not use fan_out
...
lr_array = np.array([0.1])
decay_array = np.array([0.01])
...
gpu_id = [1]
m.fit(X_train, nb_epoch=200, with_test=True, validate_data=X_valid, validate_steps=20, validate_freq=20, device=gpu_id)
