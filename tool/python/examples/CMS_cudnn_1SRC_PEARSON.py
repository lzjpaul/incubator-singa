...
from examples.datasets import CMS1SRCPEARSON
import numpy as np
import random
...
X_train, X_test, X_valid, workspace = CMS1SRCPEARSON.load_data()

version_num = random.randint(0,10000)
data_dir_prefix = '/data/zhaojing/result/2-4-CNN-CMS-1SRC-PEARSON'
...
kernel_x_param_array = np.array([6, 10, 15, 20, 25, 30, 35, 40, 60, 80])
kernel_y_param_array = np.array([2, 3])
stride_x_param_array = np.array([2, 3, 5, 8, 10])
...
if kernel_x_param == 6:
    ...
    stride_x_param = stride_x_param_array[random.randint(0,2)]
else:
    stride_x_param = stride_x_param_array[random.randint(0,len(stride_x_param_array)-1)]
