...
for dataset_id in range(train_step / batchsize):
    ...
    for i in range(x.shape[0] / batchsize):
        ...
        for h in range(1, len(nn)):
            if h == 10: # convolve
                # print "\nh == \n" , h
                print nn[h].layer.name
                nn[h].ComputeFeature(nn[h-2])
            elif h == 11: # convolve
                # print "\nh == \n", h
                print nn[h].layer.name
                nn[h].ComputeFeature(nn[h-3])
            elif h == 12: # concate
                # print "\nh == \n", h
                # print "nn[h].ComputeFeature(nn[h-1], nn[h-2], nn[h-3])"
                print nn[h].layer.name
                nn[h].ComputeFeature(nn[h-1], nn[h-2], nn[h-3])
            else:
                # print "\nh == \n", h
                print nn[h].layer.name
