...
data_dir_prefix = '/data/zhaojing/result/2-5-CMS-CNN-2layer-PEARSON-dropout'
...
input_channel = 1
filter_num_param_array = np.array([150, 200, 250, 300, 350, 400, 450])
kernel_x_param_array = np.array([3, 6, 10, 15, 20, 25, 30, 35, 40, 65, 80, 100])
...
for i in range(layer_num):
    if i == 0:
        kernel_x_param[i] = kernel_x_param_array[random.randint(0,len(kernel_x_param_array)-1)]
        kernel_y_param[i] = kernel_y_param_array[random.randint(0,len(kernel_y_param_array)-1)]
        # stride step less
        stride_x_param[i] = stride_x_param_array[random.randint(0,1)]
        stride_y_param[i] = stride_y_param_array[random.randint(1,len(stride_y_param_array)-2)] # 2
        filter_num_param[i] = filter_num_param_array[random.randint(0,4)] # 500 - 800
    elif i == 1:
        kernel_x_param[i] = kernel_x_param_array[random.randint(0,3)]
        kernel_y_param[i] = kernel_y_param_array[random.randint(0,len(kernel_y_param_array)-1)]
        stride_x_param[i] = stride_x_param_array[random.randint(0,1)] #1,2,3
        stride_y_param[i] = stride_y_param_array[random.randint(0,len(stride_y_param_array)-2)] # 1,2
        filter_num_param[i] = filter_num_param_array[random.randint(7,len(filter_num_param_array)-1)] # 500 - 1000
    else:
