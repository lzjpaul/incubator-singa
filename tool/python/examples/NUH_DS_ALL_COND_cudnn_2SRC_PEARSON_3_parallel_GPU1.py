...
# create model files
# (1) filter_num increases (2) pool to 2,2;,3,2;,3,1
# parallel: conv->relu->pool->concate
# window fix to (140,35)
import sys, os
...
data_dir_prefix = '/data/zhaojing/result/4-8-CNN-PARALLEL'
...
pad_x_param = 2
pad_y_param = 0
filter_num_param_array = np.array([50, 55, 60, 65])
...
# paralell fixed 
softmax_fan_in = 6 * filter_num_param * pool_map_y * pool_map_x # 6 times of one feature map
softmax_fan_out = 2
...
m = Nonsequential('NUHALLCOND-cnn', sys.argv)
layers_name = []
#gaussian
...
if Uniform_or_Gaussian == 1:
    layer = Convolution2D(filter_num_param, pad_x=pad_x_param, pad_y=pad_y_param, stride_x=stride_x_param, stride_y=stride_y_param, kernel_x=kernel_x_param, kernel_y=kernel_y_param, w_param=conv_parw_gaussian, b_param=conv_parb_gaussian)
    m.add(layer, ["data0"])
    layers_name.append(layer.layer.name)
elif Uniform_or_Gaussian == 0 and b_Uniform_or_Constant == 0:
    layer = Convolution2D(filter_num_param, pad_x=pad_x_param, pad_y=pad_y_param, stride_x=stride_x_param, stride_y=stride_y_param, kernel_x=kernel_x_param, kernel_y=kernel_y_param, w_param=conv_parw_uniform, b_param=conv_parb_uniform)
    m.add(layer, ["data0"])
    layers_name.append(layer.layer.name)
elif Uniform_or_Gaussian == 0 and b_Uniform_or_Constant == 1:
    layer = Convolution2D(filter_num_param, pad_x=pad_x_param, pad_y=pad_y_param, stride_x=stride_x_param, stride_y=stride_y_param, kernel_x=kernel_x_param, kernel_y=kernel_y_param, w_param=conv_parw_uniform, b_param=conv_parb_uniform_1)
    m.add(layer, ["data0"])
    layers_name.append(layer.layer.name)

layer = Activation('relu')
m.add(layer, [layers_name[0]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[1]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[3]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[4]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[6]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[7]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[9]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[10]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[12]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[13]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[15]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[16]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[18]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[19]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[21]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[22]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[24]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[25]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[27]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[28]])
layers_name.append(layer.layer.name)
...
layer = Activation('relu')
m.add(layer, [layers_name[30]])
layers_name.append(layer.layer.name)

layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[31]])
layers_name.append(layer.layer.name)
...
# pool_y_param = 2
layer = MaxPooling2D(pool_size=(pool_x_param,pool_y_param), stride=pool_stride_x_param)
m.add(layer, [layers_name[34]])
layers_name.append(layer.layer.name)

################concate layer############################
layer = Concate(dim=3, num=12)
m.add(layer, [layers_name[2], layers_name[5], layers_name[8], layers_name[11], layers_name[14], layers_name[17], layers_name[20], layers_name[23], layers_name[26], layers_name[29], layers_name[32], layers_name[35]])
layers_name.append(layer.layer.name)
...
if Uniform_or_Gaussian == 1:
    layer = Dense(2, w_param=softmax_parw_gaussian, b_param=softmax_parb_gaussian)
    m.add(layer, [layers_name[36]])
    layers_name.append(layer.layer.name)
elif Uniform_or_Gaussian == 0 and b_Uniform_or_Constant == 0:
    layer = Dense(2, w_param=softmax_parw_uniform, b_param=softmax_parb_uniform)
    m.add(layer, [layers_name[36]])
    layers_name.append(layer.layer.name)
elif Uniform_or_Gaussian == 0 and b_Uniform_or_Constant == 1:
    layer = Dense(2, w_param=softmax_parw_uniform, b_param=softmax_parb_uniform_1)
    m.add(layer, [layers_name[36]])
    layers_name.append(layer.layer.name)

layer = Activation('softmax')
m.add(layer, [layers_name[37], "data0"])
layers_name.append(layer.layer.name)
